...
*/
function AMI(_hostname, _port, _username, _password) {
  const me = this;
  const hostname = _hostname || null;
  const port = _port || null;
  const username = _username || null;
  const password = _password || null;
  ...
  */
  this.action = function (_name, _data, _cb) {
    const name = _name || '';
    const data = _data || {};
    const dataJson = {};
    
    let dataTxt = '';
    const cb = (typeof _cb == 'function') ? _cb : new Function();
